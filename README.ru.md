...
convertr проверяет `$PATH` при старте и **исключает из графа конвертации
те возможности, бинарники для которых не установлены**. Если два бэкенда
объявляют одно и то же ребро, Dijkstra автоматически выбирает доступный
— например, `xlsx → csv` идёт через `csvkit` при установленных
`in2csv`/`xlsx2csv` и через `libreoffice` в их отсутствие. Запустите
`convertr doctor`, чтобы увидеть, какие бинарники обнаружены, а каких
не хватает.
